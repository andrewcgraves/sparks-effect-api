...
| **TransitGraph** | The compile output, and the only thing an isochrone is ever plotted over: per-service edge lists, the merged nodes, and a merge report. Persisted as a succeeded compile job's `result`, and travels inline on the queue message so the worker needs no database. This API compiles the graph; it does not compute isochrones. The wire types (`TransitGraph`, `Edge`, `GraphNode`, `ServiceGraph`, `TravelMode`) are defined in the contract module and re-exported from `internal/transit` |
...
| **Subtext** | The one-line descriptor an authored `UserService` shows under its name — ca-hsr's is `Electrified · High-speed rail · Greenfield`. With the name and the **description** (free prose, a few paragraphs) it is the service's prose, and what a [publication](#seeded--curated--authored--owned) copies. Both are optional and empty by default; `Validate()` bounds them at 140 and 4,000 characters as `max_length` faults. `subtext` everywhere — column, JSON, TypeScript — not *subtitle* or *tagline*. See [ADR-0005](docs/adr/0005-publishing-an-authored-service.md) |
