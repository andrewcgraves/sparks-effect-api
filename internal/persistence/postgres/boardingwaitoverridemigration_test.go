...
// snapmigration_test.go does. 00021–00023 sit above it, so the tail is now
// rewindCAHSRRoutingAnchorsMigration. Goose refuses to re-apply an earlier
// migration while a later version is still recorded, so anything rewinding a
// migration below this one must unrecord all four, which it gets by calling
// this.
//
...
func TestBoardingWaitOverridesMigrationIsSafeToReRun(t *testing.T) {
	...
	//
	// 00021–00023 are unrecorded too, and their schema/data deliberately left
	...
	// three are re-runnable over what they already wrote by construction —
	// 00021 drops each constraint before adding it, 00022 is IF NOT EXISTS
	// throughout, 00023 re-assigns the same UPDATE literals.
	exec(t, url, `DELETE FROM goose_db_version WHERE version_id IN (20, 21, 22, 23)`)
	if err := postgres.Migrate(context.Background(), url); err != nil {
