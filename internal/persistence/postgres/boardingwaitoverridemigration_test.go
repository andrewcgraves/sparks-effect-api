...
// rewindBoardingWaitOverrideMigration unwinds 00020, unwinding the migration
// above it first the way every link in the rewind chain that starts in
// snapmigration_test.go does. Goose refuses to re-apply an earlier migration
// while a later version is still recorded, so anything rewinding a migration
// below this one must unrecord 00020 first —
...
func TestBoardingWaitOverridesMigrationIsSafeToReRun(t *testing.T) {
	...
	//
	// 00021 is unrecorded too, because goose refuses to re-apply 00020 while a
	// later version is still recorded. Both are bare DELETEs rather than calls
	// to the rewind helpers: keeping the schema in place is the whole point of
	// this test, and 00021 is idempotent in the same way, so it re-applies as a
	// no-op alongside 00020.
	exec(t, url,
