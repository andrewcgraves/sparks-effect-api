...
// it first the way every link in the rewind chain that starts in
// snapmigration_test.go does. Goose refuses to re-apply a migration older than
// the highest version recorded, so anything rewinding a migration below this
// one must unrecord 00021 first — rewindBoardingWaitOverrideMigration does that
// by calling this, and this calls rewindOwnedDomainModelsMigration for 00022.
//
...
func TestTravelModeCheckMigrationIsSafeToReRun(t *testing.T) {
	...
	//
	// 00022 is unrecorded too, and its schema deliberately left in place. Not
	// for its own sake but for this test's: goose refuses to run while a version
	// below the highest recorded one is missing, so leaving 00022 recorded turns
	// the re-run into "found 1 missing migrations before current version 22"
	// rather than the re-application this test exists to check. 00022 is
	// re-runnable over its own objects by construction — every statement in it
	// is IF NOT EXISTS — so it re-applies as a no-op alongside 00021.
